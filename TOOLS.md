...
### Gmail OAuth (`gog`) 📬
- **Purpose:** native Gmail / Google Workspace CLI auth for mailbox access and account-specific workflows
- **Binary:** `gog`
- **Current auth shape:** multiple OAuth client buckets (`default` for the main mailbox and `college` for the college mailbox) with per-account refresh tokens in the file-backed keyring
- **Client credentials:** `~/.config/gogcli/credentials.json`
- **Token storage:** encrypted file keyring under `~/.config/gogcli/keyring/` when `keyring_backend=file`
- **Current main Gmail account:** `tony.guirguis.dell@gmail.com`
- **College/client bucket:** `college` -> `tonymasoudnyc@gmail.com`
- **Account targeting:** use `--account <email>` or `GOG_ACCOUNT=<email>`; use `--account auto` only when you want default/single-token behavior
- **Multi-client support:** `--client <name>` / `GOG_CLIENT=<name>` creates isolated token buckets if a second OAuth client is ever needed
- **Read-only Gmail pattern:** `gog auth add <email> --services gmail --gmail-scope readonly`
- **Non-interactive requirement:** file backend reads cleanly when `GOG_KEYRING_PASSWORD` is set to an empty string; keep that env var available in headless/ssh runs
- **Safety rule:** keep the first Gmail account untouched; add the second account as a separate token entry and target it explicitly with `--account` or `GOG_ACCOUNT`
- **Do not use for:** `openclawmainemail@agentmail.to` (that inbox is AgentMail, not Gmail/OAuth)
...
- **Skill path:** `/root/.openclaw/workspace/skills/git-essentials/SKILL.md`
- **Purpose:** canonical Git command/workflow reference for native repo work
- **Use this for:** `git status`, branch flows, merges, rebases, stashing, reflog recovery, remote sync, and safe undo operations
- **Native rule:** execute with the system `git` binary directly; use the skill as the instruction layer, not as a wrapper
- **Agent note:** Volt should treat this as the standard Git reference when doing repo/deployment work
...
- **Alternative**: Use 2-hour inbox check for now

### College Monitoring Cron Jobs ⏰
- **Inbox Check**: Every 2 hours (`30 */2 * * *`)
...
- **Skill path:** `/root/.openclaw/workspace/skills/polymarket-trade/SKILL.md`
- **Native command:** `polymarket` (`/root/.local/bin/polymarket`)
- **Use this for:** Polymarket odds, trending markets, market search, movers, resolution calendars, watchlists, alerts, category digests, and local paper-trading simulation.
- **Safety rule:** This is read-only market data plus local paper trading only. It does not use wallets, credentials, API keys, or real-money trade execution.

### Agent Browser Skill 🌐
- **Installed:** 2026-05-30
- **Skill path:** `/root/.openclaw/workspace/skills/agent-browser-clawdbot/SKILL.md`
- **Native command:** `agent-browser` (`/usr/bin/agent-browser`, version 0.27.0)
- **Browser:** Chrome for Testing 149.0.7827.54 at `/root/.agent-browser/browsers/chrome-149.0.7827.54/chrome`
- **State namespace:** put workflow state under `~/.agent-browser/openclaw/<site-or-skill>/`; saved auth state/cookies are sensitive and should not be committed or pasted into logs.
- **Use this for:** interactive web workflows, JavaScript-rendered pages, deterministic accessibility snapshots/refs, form fills, login/session reuse, cookies/storage, network blocking, multi-tabs, and isolated multi-session workflows.
- **Priority rule:** prefer purpose-built APIs/skills first (`github`, `polymarket-trade`, `tavily`, `summarize`, `web_fetch`) for stable read-only data. Use `agent-browser` when the page must render in a browser, needs clicks/forms/cookies/auth/session state, or an API/search result is insufficient. Use OpenClaw's built-in browser tool mainly for visual inspection, screenshots/PDF-oriented analysis, or connected browser profiles.
- **Operational rule:** after every navigation or interaction, run `agent-browser wait --load networkidle` when appropriate, then `agent-browser snapshot -i --json`; refs such as `@e1` are snapshot-local and stale after page changes.
- **Known limitation:** `network route --abort` works. In local testing, `network route --body` registered but did not override test `fetch()` responses, so verify mocking before relying on it.

### Session Logs 📜
- **Enabled:** 2026-05-30
- **Skill path:** `/root/.openclaw/workspace/skills/session-logs/SKILL.md`
- **Main agent ID:** `main`
- **Main session path:** `/root/.openclaw/agents/main/agent/codex-home/sessions`
- **Requirements:** `jq` (`/usr/bin/jq`) and `rg` (OpenClaw/Codex vendored `rg`) are available and pass the skill requirement check.
- **Use this for:** raw past conversation lookup, exact previous wording, old tool calls, session token/cost summaries, and searches across JSONL transcripts.
- **Routing rule:** `session-logs` owns raw transcript/history questions. `MEMORY.md`, daily memory files, QMD, and ontology own distilled long-term facts and remembered preferences.
- **Privacy rule:** session logs contain injected runtime context, tool outputs, emails, and possible secrets. Search narrowly, summarize results, and do not dump raw JSONL or credentials into chat.
...
- **Auto-updater + auto-heal maintenance:** `scripts/openclaw-auto-updater.sh` is the native host updater, scheduled from root crontab at 02:45 America/New_York. It holds `/var/lock/openclaw-auto-maintenance.lock`, runs `openclaw update --yes`, `openclaw doctor --fix --yes --non-interactive`, `clawhub --workdir /root/.openclaw/workspace --no-input update --all`, then runs `scripts/daily-maintenance.sh` with `AUTO_UPDATE=0` for verification. `scripts/daily-maintenance.sh` also honors the same lock when run independently. The OpenClaw `daily-maintenance` cron runs at 04:00 America/New_York as the review/report/follow-up pass. Risky fixes still report instead of blindly changing firewall/routes/auth/destructive data.
