...
Use `session-logs` as the authoritative source for raw past conversation content, exact wording, prior tool calls, and session token/cost summaries. Use `MEMORY.md`, daily memory files, QMD, and ontology for distilled facts and long-term remembered context. Search narrowly and summarize raw logs; do not dump full transcripts or secrets into chat.
...
## Weekly Self‑Improvement Update (2026‑05‑24)
- **Code Patterns:** No weekly issues recorded yet. Treat the learning file as the source of truth; do not invent patterns when the log is empty.
- **Feedback:** No new Tony reactions captured. Keep tracking exact phrasing and format complaints when they appear.
- **Stack Evolution:** No new stack signals recorded. Avoid speculative stack assumptions until the file shows repeated entries.
- **Model Usage:** No sign of ChatGPT 5.4 mini overuse in the logs. Use `gpt-5.4-mini` for fast synthesis and routine analysis; reserve heavier reasoning only when the task actually needs it.
- **Learnings:**
  - Sparse logs mean the weekly update should be conservative and evidence-based.
  - If the logs stay empty, summarize that directly instead of padding with inferred trends.
  - Routing should stay unchanged unless repeated feedback shows a real mismatch.
- **Actions:**
  - Keep logging concrete code-pattern issues, Tony feedback, and stack changes with dates.
  - Prefer short, factual weekly summaries when the learning files have little data.
  - Escalate model choice only when the task has enough complexity to justify it.
- **Review Style Adjustments:**
  - State whether evidence exists before drawing conclusions.
  - Lead with the top 1-3 concrete changes, then note absences.
  - Keep review comments terse and operational.
- **Briefing Content Updates:**
  - Include a quick status line for each learning file.
  - Call out whether model usage is intentional or merely habitual.
  - Mention routing changes only when there is an actual change.
- **Message Format:**
  - Keep `(system)` as the prefix for system updates.
  - Use one compact sentence or semicolon-separated clause list for Telegram.
  - Avoid extra formatting when the update is only a status note.
- **Routing Adjustments:** None. Continue the current Oboz/Volt/Mega split until the logs provide evidence for a change.
