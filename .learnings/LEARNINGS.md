...
## 2026-05-30
- Stony Brook News still exposes a Mailchimp form, but the cron needed a desktop browser user-agent to avoid 403 responses from `requests`.
- Stony Brook Matters has moved to a Google Form, so the automation needs a separate submit path instead of reusing the Mailchimp parser.
- CCNY news is currently blocked by Cloudflare in the browser, so that branch still needs either a different access path or a different expectation.
